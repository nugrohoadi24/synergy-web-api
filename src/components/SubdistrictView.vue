<template>
  <b-container fluid>
    ...
    <b-modal ref="formEdit" hide-header size="lg" scrollable lazy centered 
      ...
      <MyInputText label="KODE KELURAHAN" :value.sync="currentData.code" mx=10 mandatory/> 
      <MyInputText label="NAMA KELURAHAN" :value.sync="currentData.name" mx=150 mandatory/> 
      <b-row class="editRow" >
        <b-col sm="3">
          <label>ACTIVE</label>
        </b-col>
  ...
  export default {
    data () {
      return {
        ...
        loading: false,
        fields: [
          { key: 'code', label:"VILLAGE CODE", sortable: true},
    ...
    methods: {
      ...
      async onDelete(data){
        this.$emit('showLoading', true);
        var response = await this.$http('delete', `/subdistrict/${data._id}`);
        this.$emit('showLoading', false);
      ...
      async saveData(bvModalEvt){
        ...
        this.$emit('showLoading', true);
        var response = await this.$http(Boolean(this.currentData._id)?"post":"put", `/subdistrict`,this.currentData);
        this.$emit('showLoading', false);
