<template>
  <b-container fluid>
    ...
    <b-modal ref="formEdit" hide-header size="lg" scrollable  lazy  centered 
      ...
      <MyInputText label="KODE KOTA" :value.sync="currentData.code" mx=10 mandatory/> 
      <MyInputText label="NAMA KOTA" :value.sync="currentData.name" mx=150 mandatory/> 
  ...
  export default {
    data () {
      return {
        ...
        loading: false,
        fields: [
          { key: 'code', label:"CITY CODE", sortable: true},
    ...
    methods: {
      ...
      async onDelete(data){
        this.$emit('showLoading', true);
        var response = await this.$http('delete', `/city/${data._id}`);
        this.$emit('showLoading', false);
      ...
      async saveData(bvModalEvt){
        ...
        if(!Boolean(this.currentData.code))
          errmsg += "* Silahkan input kode kota \r\n";

        if(!Boolean(this.currentData.name))
          errmsg += "* Silahkan input nama kota \r\n";

        if(!Boolean(this.currentData.province))
          errmsg += "* Silahkan Pilih Provinsi dari kota \r\n";
        ...
        this.$emit('showLoading', true);
        var response = await this.$http(Boolean(this.currentData._id)?"post":"put", `/city`,this.currentData);
        this.$emit('showLoading', false);
