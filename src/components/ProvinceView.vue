<template>
  <b-container fluid>
    ...
    <b-modal ref="formEdit" hide-header size="lg" scrollable  lazy centered 
      ...
      <MyInputText label="KODE PROVINSI" :value.sync="currentData.code" mx=10 mandatory/> 
      <MyInputText label="NAMA PROVINSI" :value.sync="currentData.name" mx=150 mandatory/> 
  ...
  export default {
    data () {
      return {
        ...
        loading: false,
        fields: [
          { key: 'code', label:"PROVINCE CODE", sortable: true,  },
    ...
    methods: {
      ...
      async onDelete(data){
        this.$emit('showLoading', true);
        var response = await this.$http('delete', `/province/${data._id}`);
        this.$emit('showLoading', false);
      ...
      async saveData(bvModalEvt){
        ...
        if(!Boolean(this.currentData.code))
          errmsg += "* Silahkan input code provinsi  \r\n";

        if(!Boolean(this.currentData.name))
          errmsg += "* Silahkan input nama provinsi \r\n";
        ...
        this.$emit('showLoading', true);
        var response = await this.$http(Boolean(this.currentData._id)?"post":"put", `/province`,this.currentData);
        this.$emit('showLoading', false);
